...
class node:
    def __init__(self,lr,hidden_number,hidden_node):
        ...
        self.wq_4=np.random.randn(hidden_node,hidden_node,hidden_number)*np.sqrt(1/hidden_node)
        self.w4_3=np.random.randn(hidden_node,hidden_node,hidden_number)*np.sqrt(1/hidden_node)
    def relu(self,value):
    ...
    def forward(self,h_o,i):
        ...
        self.h_o4[:,i]=self.tanh(self.h_i4[:,i])#
        #汇入
        self.h_i3[:,i]=0
    ...
    def backward(self,dh,j):
        ...
        self.dh_i4=self.dh_o4*self.tanh_f(self.h_i4[:,j])
